...
public partial class ConsoleViewModel : ViewModel, IConsole
{
    [Inject] private InputManager _inputManager;

    private InputAction _toggleAction;
    private bool _consoleOppenned = false;
    private sbyte _historyIndex = -1;
    private string _currentInput;
    private IDisposable _inputObservation;
    ...
    public ReactiveCommand<bool> switchConsoleVisible = new();
    public ReactiveCommand<string> navigateHistory = new();
    public ReactiveCommand<string> autoComplete = new();
    public ReactiveCommand<string> updateOutputDisplay = new();
    ...
    public void Initialize(int maxOutputLines)
    {
    ...
    private void SetupInput()
    {
        ...
        onDisposeActions.Add(() => _toggleAction.performed += OnInputAction);
    }

    private void OnInputAction(InputAction.CallbackContext ctx)
    {
        _consoleOppenned = !_consoleOppenned;
        switchConsoleVisible.Execute(_consoleOppenned);
    ...
    private void NavigateHistory(sbyte direction)
    {
        if (_commandHistory.Count == 0) return;

        _historyIndex = (sbyte)Mathf.Clamp(_historyIndex + direction, -1, _commandHistory.Count - 1);

        if (_historyIndex == -1)
        {
            navigateHistory.Execute(_currentInput);

        }
        else
        {
            navigateHistory.Execute(_commandHistory[_historyIndex]);
        }
    }

    private void StartInputObservation()
    {
        StopInputObservation();

        _inputObservation = Observable.EveryUpdate()
            .Subscribe(_ => HandleNavigationInput())
            .AddTo(disposables);
    }

    private void StopInputObservation()
    {
        _inputObservation?.Dispose();
        _inputObservation = null;
    }
    ...
    private void AddOutput(string message)
    {
        ...
        updateOutputDisplay.Execute(string.Join("\n", _outputLines));
    ...
    private void AutoComplete()
    {
        ...
        var matches = _commands.Keys.Where(cmd => cmd.StartsWith(_currentInput, StringComparison.OrdinalIgnoreCase)).ToList();

        if (matches.Count == 1)
        {
            autoComplete.Execute(matches[0] + " ");
        }
