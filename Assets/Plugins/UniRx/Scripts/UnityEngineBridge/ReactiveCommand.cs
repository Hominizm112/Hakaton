...
namespace UniRx
{
    ...
    public class ReactiveCommand<T> : IReactiveCommand<T>, IDisposable
    {
        ...
        public static implicit operator ReactiveCommand<T>(int v)
        {
            ...
            if (canExecute.Value)
            {
                ...
                if (a.Length == 1)
                {
                    try
                    {
                        ...
                        return asyncState.Finally(() => canExecuteSource.Value = true).Subscribe(asyncState);
                    }
                    catch
                    {
                ...
                else
                {
                    ...
                    return Observable.WhenAll(xs).Finally(() => canExecuteSource.Value = true).Subscribe(xs);
                }
            }
            else
            {
